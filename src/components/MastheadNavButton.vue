...
<template>
	<AbstractButton v-slot="{ isActive, href, navigate }" custom>
		<component :is="href ? 'a': 'button'" :href="href" v-bind="$attrs" @click="navigate" class="py-3/4 xs:px-1 md:py-1/2 md:px-1/2 flex flex-col items-center gap-1/4" :class="{ 'text-white bg-neutral-800 mouse:hover:bg-neutral-750 active:bg-neutral-850': !active && !isActive, [activeClass]: active || isActive }">
